...
def getData(fcur, tcur, delta_time, start_day, current_time=None):
    ...
    if current_time - start_day <= datetime.timedelta(days=PERIOD_WITH_HOURLY_CHUNK):
        ...
        data = list(map(lambda x,y: [x[0], x[1]/y[1]], fprice_btc, tprice_btc))
...
def makeMarketStats(market, interval, percent, period, rate=None, get_data=False):
    ...
    rates = [current_rate]
    while rates[len(rates)-1] + current_rate * ratio < max(data, key=lambda x: x[1])[1]:
        rates.append(rates[len(rates)-1] + current_rate * ratio)
    rates.append(max(data, key=lambda x: x[1])[1])

    while rates[0] - current_rate * ratio > min(data, key=lambda x: x[1])[1]:
        rates.insert(0, rates[0] - current_rate * ratio)
    if min(data, key=lambda x: x[1])[1] < current_rate:
        rates.insert(0, min(data, key=lambda x: x[1])[1])
